...
| 模型服务 | 管理模型和 AI 能力配置 | LLM 配置、Embedding 配置、模型测试、调用记录 | P1 | 实现配置存储、激活状态和连接测试 |
...
| 页面 | 路由 | 主要组件 | MVP 要求 |
| --- | --- | --- | --- |
| 知识中心 | `/knowledge` | Tab（案例库/运维手册/文档管理）、类型筛选、语义搜索、上传弹窗、查看弹窗、删除操作 | 文档、案例、Runbook 的 CRUD 管理与语义检索 |
| 对话运维 | `/chatops` | 对话窗口、故障诊断输入、结构化结果卡片、Agent 执行轨迹、根因候选、处置计划 | 支持查询、诊断、Runbook 检索和流程触发（原独立 `/diagnosis` 已合并） |
